...
class PeerTabModel with ChangeNotifier {
  ...
  // Deploy customizado: recent/fav/lan (e ab/group) ficam ocultos por padrão.
  // Só são religados via o menu "Acesso avançado" (senha), ver setTabEnabled().
  List<bool> isEnabled = List.filled(maxTabCount, false, growable: false);
  ...
  PeerTabModel(this.parent) {
    // enabled (deploy customizado: religado só via menu "Acesso avançado")
    try {
      final option = bind.getLocalFlutterOption(k: kOptionPeerTabEnabledCustom);
      if (option.isNotEmpty) {
        List<dynamic> decodeList = jsonDecode(option);
        if (decodeList.length == isEnabled.length) {
          for (int i = 0; i < isEnabled.length; i++) {
            if (decodeList[i] is bool) {
              isEnabled[i] = decodeList[i];
            }
          }
        }
      }
    } catch (e) {
      debugPrint("failed to get peer tab enabled list: $e");
    }
    // visible
    try {
  ...
  // Chamado só a partir do diálogo "Acesso avançado" (senha n041vcdb).
  setTabEnabled(int index, bool enabled) {
    if (index >= 0 && index < maxTabCount) {
      if (isEnabled[index] != enabled) {
        isEnabled[index] = enabled;
        if (index == _currentTab && !enabled) {
          _trySetCurrentTabToFirstVisibleEnabled();
        } else if (enabled && visibleEnabledOrderedIndexs.length == 1) {
          _currentTab = index;
        }
        try {
          bind.setLocalFlutterOption(
              k: kOptionPeerTabEnabledCustom, v: jsonEncode(isEnabled));
        } catch (_) {}
        notifyListeners();
      }
    }
